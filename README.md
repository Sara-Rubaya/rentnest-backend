...
- **Stripe CLI** (recommended): `stripe listen --forward-to localhost:5000/api/payments/confirm`, then use the printed webhook signing secret as `STRIPE_WEBHOOK_SECRET`.
...
5. Set your Stripe webhook endpoint in the Stripe dashboard to `https://<your-render-url>/api/payments/confirm` and copy the signing secret into `STRIPE_WEBHOOK_SECRET`.
